...
results_frame <- data.frame(layer = c(-999))
# for each layer
...
for (i in 1:length(kr_files)){
  ...
  for (lang in c("CMN","SHS","KR")){
    ...
    results_row <- data.frame(dimensionality = c(d),
    ...
    if (results_frame$layer[1] == -999){
      results_frame <- results_row
      } else
        results_frame <- rbind(results_frame,results_row)
    
    # Save results
    write.csv(results_frame,"distance_ratio_L1vL2_Main.csv",row.names=F)
  } # for each language
  }# for layer
